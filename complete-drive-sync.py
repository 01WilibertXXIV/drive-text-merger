...
def process_documents(service, start_time, doc_db, target_id=None, target_type=None):
    ...
    # Add target folder constraint if specified
    if target_id and target_type == 'folder':
        if target_id == "root":
            query += " and 'root' in parents"
        else:
            query += f" and '{target_id}' in parents"
    elif target_id and target_type == 'file':
        query = f"id='{target_id}'"  # Override query to just get this file
    elif target_id and target_type == 'drive':
        # Don't add any constraints for shared drive, we'll use the driveId parameter instead
        pass
    ...
    try:
        ...
        # Add driveId parameter if targeting a shared drive
        if target_type == 'drive':
            list_params['corpora'] = 'drive'
            list_params['driveId'] = target_id
            list_params['includeItemsFromAllDrives'] = True
            list_params['supportsAllDrives'] = True
        
        # If targeting a shared folder or file that might be in a shared drive
        if target_type in ['folder', 'file']:
            list_params['includeItemsFromAllDrives'] = True
            list_params['supportsAllDrives'] = True
        ...
        while True:
            ...
            logging.info(f"Found {len(items)} files in this batch")
            print(f"Found {len(items)} files in this batch")
